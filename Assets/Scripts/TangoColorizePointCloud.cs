...
/// functionality.
/// </summary>
public class TangoColorizePointCloud : MonoBehaviour, ITangoPointCloud
{
	/// <summary>
	...
	public List<Vector3> m_nrPoints;

	[HideInInspector]
	public List<Color32> m_pointColor;
	public List<Color32> m_nrColor;
	...
	public void Start()
	{
		...
		m_pointColor = new List<Color32>();
		m_nrColor = new List<Color32>();
...
/*
				...
				for (int i = 0; i < m_nrPoints.Count; ++i)
				{
					indices[i] = i;
				}

				m_mesh.Clear();
				m_mesh.vertices = m_nrPoints.ToArray();
				m_mesh.colors32 = m_nrColor.ToArray();
...
*/
				...
				m_mesh.colors32 = m_pointColor.ToArray();
	...
	public bool EstimateDepthColor(Camera cam)
	{
		...
		{
			// Set up parameters
//			Matrix4x4 colorCameraTUnityWorld = Matrix4x4.Translate(new Vector3(-0.01f, 0.05f, 0)) * TangoSupport.COLOR_CAMERA_T_UNITY_CAMERA * cam.transform.worldToLocalMatrix;
			Matrix4x4 colorCameraTUnityWorld = TangoSupport.COLOR_CAMERA_T_UNITY_CAMERA * cam.transform.worldToLocalMatrix;

			//			Debug.Log(TangoSupport.DEVICE_T_UNITY_CAMERA);

			for (int i = 0; i < m_points.Length; i++)
			{
				if (i < m_pointsCount)
				{
					Vector3 cameraPoint = m_points[i];
					cameraPoint = colorCameraTUnityWorld.MultiplyPoint(cameraPoint);

					if (cameraPoint.z > 0.05f)
					{
						Vector2 screenPos = new Vector2(cameraPoint.y / cameraPoint.z + (float)((m_colorCameraIntrinsics.cx) / m_colorCameraIntrinsics.fx),
														-cameraPoint.x / cameraPoint.z + (float)((m_colorCameraIntrinsics.cy) / m_colorCameraIntrinsics.fy));

//						screenPos = arScreen.ViewportPointToCameraImagePoint(screenPos);
						screenPos.x *= (float)m_colorCameraIntrinsics.fx;
						screenPos.y *= (float)m_colorCameraIntrinsics.fy;

						m_pointColor.Add(m_tangoColorBytes.PickColor(screenPos.x, screenPos.y));
					}
					else
					{
						m_pointColor.Add(new Color32(0, 0, 0, 255));
					}
				}
				else
				{
						...
						out Vector3 colorCameraPoint)
	{
		...
		int returnValue = TangoSupport.TangoSupportAPI.TangoSupport_calculateRelativePose(
		...
		returnValue = TangoSupport.TangoSupportAPI.TangoSupport_getDepthAtPointNearestNeighbor(
